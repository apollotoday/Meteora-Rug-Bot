...
var _bufferlayout = require('@solana/buffer-layout'); var BufferLayout = _interopRequireWildcard(_bufferlayout);
...
var _browser = require('jayson/lib/client/browser'); var _browser2 = _interopRequireDefault(_browser);
var _nodefetch = require('node-fetch'); var nodeFetch = _interopRequireWildcard(_nodefetch);
...
var generatePrivateKey = _ed25519.ed25519.utils.randomPrivateKey;
var generateKeypair = () => {
  const privateScalar = _ed25519.ed25519.utils.randomPrivateKey();
...
var getPublicKey = _ed25519.ed25519.getPublicKey;
function isOnCurve(publicKey2) {
  try {
    _ed25519.ed25519.ExtendedPoint.fromHex(publicKey2);
    return true;
  } catch (e2) {
    return false;
  }
}
var sign = (message, secretKey) => _ed25519.ed25519.sign(message, secretKey.slice(0, 32));
var verify = _ed25519.ed25519.verify;
var toBuffer = (arr) => {
...
var Struct = class {
  ...
  encode() {
    return _buffer.Buffer.from(_borsh.serialize.call(void 0, SOLANA_SCHEMA, this));
  }
  static decode(data) {
    return _borsh.deserialize.call(void 0, SOLANA_SCHEMA, this, data);
  }
  static decodeUnchecked(data) {
    return _borsh.deserializeUnchecked.call(void 0, SOLANA_SCHEMA, this, data);
  }
};
...
var PublicKey = class extends Struct {
  ...
  constructor(value) {
    ...
    if (isPublicKeyData(value)) {
      ...
      if (typeof value === "string") {
        const decoded = _bs582.default.decode(value);
        if (decoded.length != PUBLIC_KEY_LENGTH) {
          throw new Error(`Invalid public key input`);
        }
        this._bn = new (0, _bnjs2.default)(decoded);
      } else {
        this._bn = new (0, _bnjs2.default)(value);
      }
   ...
   */
  toBase58() {
    return _bs582.default.encode(this.toBytes());
  }
  toJSON() {
  ...
  static async createWithSeed(fromPublicKey, seed, programId) {
    ...
    const publicKeyBytes = _sha256.sha256.call(void 0, buffer);
...
function guardedSplice(byteArray, ...args) {
  const [start] = args;
  if (args.length === 2 ? start + (_nullishCoalesce(args[1], () => ( 0))) > byteArray.length : start >= byteArray.length) {
    throw new Error(END_OF_BUFFER_ERROR_MESSAGE);
  }
...
var Message = class {
  ...
  get compiledInstructions() {
    return this.instructions.map((ix) => ({
      ...
      data: _bs582.default.decode(ix.data)
    }));
  }
  ...
  static compile(args) {
    ...
    const instructions = accountKeys.compileInstructions(args.instructions).map((ix) => ({
      ...
      data: _bs582.default.encode(ix.data)
    }));
    return new Message({
      header,
  ...
  serialize() {
    ...
    const instructions = this.instructions.map((instruction) => {
      ...
      const data = Array.from(_bs582.default.decode(instruction.data));
      let keyIndicesCount = [];
    ...
    const transaction = {
      ...
      recentBlockhash: _bs582.default.decode(this.recentBlockhash)
    };
  ...
  static from(buffer) {
    ...
    for (let i = 0; i < instructionCount; i++) {
      ...
      const data = _bs582.default.encode(_buffer.Buffer.from(dataSlice));
      instructions.push({
        programIdIndex,
        accounts,
        data
    ...
    const messageArgs = {
      ...
      recentBlockhash: _bs582.default.encode(_buffer.Buffer.from(recentBlockhash)),
      accountKeys,
      instructions
    };
...
var DEFAULT_SIGNATURE = _buffer.Buffer.alloc(SIGNATURE_LENGTH_IN_BYTES).fill(0);
  ...
  compileMessage() {
    ...
    const compiledInstructions = instructions.map((instruction) => {
      ...
      return {
        ...
        data: _bs582.default.encode(data)
      };
    });
...
Missing signature for public key${sigErrors.missing.length === 1 ? "" : "(s)"} [\`${sigErrors.missing.map((p) => p.toBase58()).join("`, `")}\`].`;
  ...
  static from(buffer) {
    ...
    for (let i = 0; i < signatureCount; i++) {
      ...
      signatures.push(_bs582.default.encode(_buffer.Buffer.from(signature)));
    }
  ...
  static populate(message, signatures = []) {
    ...
    signatures.forEach((signature, index) => {
      const sigPubkeyPair = {
        signature: signature == _bs582.default.encode(DEFAULT_SIGNATURE) ? null : _bs582.default.decode(signature),
    ...
    message.instructions.forEach((instruction) => {
      ...
      transaction.instructions.push(new TransactionInstruction({
        ...
        data: _bs582.default.decode(instruction.data)
      }));
    });
...
async function sendAndConfirmTransaction(connection, transaction, signers, options) {
  ...
  if (transaction.recentBlockhash != null && transaction.lastValidBlockHeight != null) {
    status = (await connection.confirmTransaction({
      abortSignal: _optionalChain([options, 'optionalAccess', _2 => _2.abortSignal]),
      signature,
    ...
    status = (await connection.confirmTransaction({
      abortSignal: _optionalChain([options, 'optionalAccess', _3 => _3.abortSignal]),
    ...
    if (_optionalChain([options, 'optionalAccess', _4 => _4.abortSignal]) != null) {
...
var bigInt = (length) => (property) => {
  const layout = _bufferlayout.blob.call(void 0, length, property);
  const {
    encode,
    decode
  } = encodeDecode(layout);
  ...
  bigIntLayout.decode = (buffer, offset2) => {
    ...
    return _bigintbuffer.toBigIntLE.call(void 0, _buffer.Buffer.from(src));
  };
  bigIntLayout.encode = (bigInt2, buffer, offset2) => {
    const src = _bigintbuffer.toBufferLE.call(void 0, bigInt2, length);
...
var Loader = class {
  ...
  static async load(connection, payer, program, programId, data) {
    ...
    while (array2.length > 0) {
      ...
      const data2 = _buffer.Buffer.alloc(chunkSize + 16);
      dataLayout.encode({
        instruction: 0,
        // Load instruction
        offset: offset2,
    ...
    await Promise.all(transactions);
    {
      ...
      const data2 = _buffer.Buffer.alloc(dataLayout2.span);
      dataLayout2.encode({
        instruction: 1
...
var PublicKeyFromString = _superstruct.coerce.call(void 0, _superstruct.instance.call(void 0, PublicKey), _superstruct.string.call(void 0, ), (value) => new PublicKey(value));
var RawAccountDataResult = _superstruct.tuple.call(void 0, [_superstruct.string.call(void 0, ), _superstruct.literal.call(void 0, "base64")]);
var BufferFromRawAccountData = _superstruct.coerce.call(void 0, _superstruct.instance.call(void 0, _buffer.Buffer), RawAccountDataResult, (value) => _buffer.Buffer.from(value[0], "base64"));
var BLOCKHASH_CACHE_TIMEOUT_MS = 30 * 1e3;
function createRpcResult(result) {
  return _superstruct.union.call(void 0, [_superstruct.type.call(void 0, {
    ...
    jsonrpc: _superstruct.literal.call(void 0, "2.0"),
    id: _superstruct.string.call(void 0, ),
    error: _superstruct.type.call(void 0, {
      code: _superstruct.unknown.call(void 0, ),
      message: _superstruct.string.call(void 0, ),
      data: _superstruct.optional.call(void 0, _superstruct.any.call(void 0, ))
    })
  })]);
}
var UnknownRpcResult = createRpcResult(_superstruct.unknown.call(void 0, ));
function jsonRpcResult(schema) {
  return _superstruct.coerce.call(void 0, createRpcResult(schema), UnknownRpcResult, (value) => {
    if ("error" in value) {
      return value;
    } else {
      return {
        ...value,
        result: _superstruct.create.call(void 0, value.result, schema)
      };
    }
  });
}
function jsonRpcResultAndContext(value) {
  return jsonRpcResult(_superstruct.type.call(void 0, {
    context: _superstruct.type.call(void 0, {
      slot: _superstruct.number.call(void 0, )
    }),
    value
  }));
}
function notificationResultAndContext(value) {
  return _superstruct.type.call(void 0, {
    context: _superstruct.type.call(void 0, {
      slot: _superstruct.number.call(void 0, )
    }),
    value
...
var GetInflationGovernorResult = _superstruct.type.call(void 0, {
  foundation: _superstruct.number.call(void 0, ),
  foundationTerm: _superstruct.number.call(void 0, ),
  initial: _superstruct.number.call(void 0, ),
  taper: _superstruct.number.call(void 0, ),
  terminal: _superstruct.number.call(void 0, )
});
var GetInflationRewardResult = jsonRpcResult(_superstruct.array.call(void 0, _superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  epoch: _superstruct.number.call(void 0, ),
  effectiveSlot: _superstruct.number.call(void 0, ),
  amount: _superstruct.number.call(void 0, ),
  postBalance: _superstruct.number.call(void 0, ),
  commission: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, )))
}))));
var GetRecentPrioritizationFeesResult = _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
  slot: _superstruct.number.call(void 0, ),
  prioritizationFee: _superstruct.number.call(void 0, )
}));
var GetInflationRateResult = _superstruct.type.call(void 0, {
  total: _superstruct.number.call(void 0, ),
  validator: _superstruct.number.call(void 0, ),
  foundation: _superstruct.number.call(void 0, ),
  epoch: _superstruct.number.call(void 0, )
});
var GetEpochInfoResult = _superstruct.type.call(void 0, {
  epoch: _superstruct.number.call(void 0, ),
  slotIndex: _superstruct.number.call(void 0, ),
  slotsInEpoch: _superstruct.number.call(void 0, ),
  absoluteSlot: _superstruct.number.call(void 0, ),
  blockHeight: _superstruct.optional.call(void 0, _superstruct.number.call(void 0, )),
  transactionCount: _superstruct.optional.call(void 0, _superstruct.number.call(void 0, ))
});
var GetEpochScheduleResult = _superstruct.type.call(void 0, {
  slotsPerEpoch: _superstruct.number.call(void 0, ),
  leaderScheduleSlotOffset: _superstruct.number.call(void 0, ),
  warmup: _superstruct.boolean.call(void 0, ),
  firstNormalEpoch: _superstruct.number.call(void 0, ),
  firstNormalSlot: _superstruct.number.call(void 0, )
});
var GetLeaderScheduleResult = _superstruct.record.call(void 0, _superstruct.string.call(void 0, ), _superstruct.array.call(void 0, _superstruct.number.call(void 0, )));
var TransactionErrorResult = _superstruct.nullable.call(void 0, _superstruct.union.call(void 0, [_superstruct.type.call(void 0, {}), _superstruct.string.call(void 0, )]));
var SignatureStatusResult = _superstruct.type.call(void 0, {
  err: TransactionErrorResult
});
var SignatureReceivedResult = _superstruct.literal.call(void 0, "receivedSignature");
var VersionResult = _superstruct.type.call(void 0, {
  "solana-core": _superstruct.string.call(void 0, ),
  "feature-set": _superstruct.optional.call(void 0, _superstruct.number.call(void 0, ))
});
var ParsedInstructionStruct = _superstruct.type.call(void 0, {
  program: _superstruct.string.call(void 0, ),
  programId: PublicKeyFromString,
  parsed: _superstruct.unknown.call(void 0, )
});
var PartiallyDecodedInstructionStruct = _superstruct.type.call(void 0, {
  programId: PublicKeyFromString,
  accounts: _superstruct.array.call(void 0, PublicKeyFromString),
  data: _superstruct.string.call(void 0, )
});
var SimulatedTransactionResponseStruct = jsonRpcResultAndContext(_superstruct.type.call(void 0, {
  err: _superstruct.nullable.call(void 0, _superstruct.union.call(void 0, [_superstruct.type.call(void 0, {}), _superstruct.string.call(void 0, )])),
  logs: _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, _superstruct.string.call(void 0, ))),
  accounts: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, _superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
    executable: _superstruct.boolean.call(void 0, ),
    owner: _superstruct.string.call(void 0, ),
    lamports: _superstruct.number.call(void 0, ),
    data: _superstruct.array.call(void 0, _superstruct.string.call(void 0, )),
    rentEpoch: _superstruct.optional.call(void 0, _superstruct.number.call(void 0, ))
  }))))),
  unitsConsumed: _superstruct.optional.call(void 0, _superstruct.number.call(void 0, )),
  returnData: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
    programId: _superstruct.string.call(void 0, ),
    data: _superstruct.tuple.call(void 0, [_superstruct.string.call(void 0, ), _superstruct.literal.call(void 0, "base64")])
  }))),
  innerInstructions: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
    index: _superstruct.number.call(void 0, ),
    instructions: _superstruct.array.call(void 0, _superstruct.union.call(void 0, [ParsedInstructionStruct, PartiallyDecodedInstructionStruct]))
  }))))
}));
var BlockProductionResponseStruct = jsonRpcResultAndContext(_superstruct.type.call(void 0, {
  byIdentity: _superstruct.record.call(void 0, _superstruct.string.call(void 0, ), _superstruct.array.call(void 0, _superstruct.number.call(void 0, ))),
  range: _superstruct.type.call(void 0, {
    firstSlot: _superstruct.number.call(void 0, ),
    lastSlot: _superstruct.number.call(void 0, )
  })
}));
...
var SlotRpcResult = jsonRpcResult(_superstruct.number.call(void 0, ));
var GetSupplyRpcResult = jsonRpcResultAndContext(_superstruct.type.call(void 0, {
  total: _superstruct.number.call(void 0, ),
  circulating: _superstruct.number.call(void 0, ),
  nonCirculating: _superstruct.number.call(void 0, ),
  nonCirculatingAccounts: _superstruct.array.call(void 0, PublicKeyFromString)
}));
var TokenAmountResult = _superstruct.type.call(void 0, {
  amount: _superstruct.string.call(void 0, ),
  uiAmount: _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, )),
  decimals: _superstruct.number.call(void 0, ),
  uiAmountString: _superstruct.optional.call(void 0, _superstruct.string.call(void 0, ))
});
var GetTokenLargestAccountsResult = jsonRpcResultAndContext(_superstruct.array.call(void 0, _superstruct.type.call(void 0, {
...
var GetTokenAccountsByOwner = jsonRpcResultAndContext(_superstruct.array.call(void 0, _superstruct.type.call(void 0, {
  pubkey: PublicKeyFromString,
  account: _superstruct.type.call(void 0, {
    executable: _superstruct.boolean.call(void 0, ),
    owner: PublicKeyFromString,
    lamports: _superstruct.number.call(void 0, ),
    data: BufferFromRawAccountData,
    rentEpoch: _superstruct.number.call(void 0, )
  })
})));
var ParsedAccountDataResult = _superstruct.type.call(void 0, {
  program: _superstruct.string.call(void 0, ),
  parsed: _superstruct.unknown.call(void 0, ),
  space: _superstruct.number.call(void 0, )
});
var GetParsedTokenAccountsByOwner = jsonRpcResultAndContext(_superstruct.array.call(void 0, _superstruct.type.call(void 0, {
  pubkey: PublicKeyFromString,
  account: _superstruct.type.call(void 0, {
    executable: _superstruct.boolean.call(void 0, ),
    owner: PublicKeyFromString,
...
var GetLargestAccountsRpcResult = jsonRpcResultAndContext(_superstruct.array.call(void 0, _superstruct.type.call(void 0, {
  lamports: _superstruct.number.call(void 0, ),
  address: PublicKeyFromString
})));
var AccountInfoResult = _superstruct.type.call(void 0, {
  executable: _superstruct.boolean.call(void 0, ),
  owner: PublicKeyFromString,
  lamports: _superstruct.number.call(void 0, ),
  data: BufferFromRawAccountData,
...
var KeyedAccountInfoResult = _superstruct.type.call(void 0, {
  pubkey: PublicKeyFromString,
  account: AccountInfoResult
});
var ParsedOrRawAccountData = _superstruct.coerce.call(void 0, _superstruct.union.call(void 0, [_superstruct.instance.call(void 0, _buffer.Buffer), ParsedAccountDataResult]), _superstruct.union.call(void 0, [RawAccountDataResult, ParsedAccountDataResult]), (value) => {
  if (Array.isArray(value)) {
    return _superstruct.create.call(void 0, value, BufferFromRawAccountData);
  } else {
    return value;
  }
});
var ParsedAccountInfoResult = _superstruct.type.call(void 0, {
  executable: _superstruct.boolean.call(void 0, ),
  owner: PublicKeyFromString,
  lamports: _superstruct.number.call(void 0, ),
...
var KeyedParsedAccountInfoResult = _superstruct.type.call(void 0, {
  pubkey: PublicKeyFromString,
  account: ParsedAccountInfoResult
});
var StakeActivationResult = _superstruct.type.call(void 0, {
  state: _superstruct.union.call(void 0, [_superstruct.literal.call(void 0, "active"), _superstruct.literal.call(void 0, "inactive"), _superstruct.literal.call(void 0, "activating"), _superstruct.literal.call(void 0, "deactivating")]),
  active: _superstruct.number.call(void 0, ),
  inactive: _superstruct.number.call(void 0, )
});
var GetConfirmedSignaturesForAddress2RpcResult = jsonRpcResult(_superstruct.array.call(void 0, _superstruct.type.call(void 0, {
  signature: _superstruct.string.call(void 0, ),
  slot: _superstruct.number.call(void 0, ),
  err: TransactionErrorResult,
  memo: _superstruct.nullable.call(void 0, _superstruct.string.call(void 0, )),
  blockTime: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, )))
})));
var GetSignaturesForAddressRpcResult = jsonRpcResult(_superstruct.array.call(void 0, _superstruct.type.call(void 0, {
  signature: _superstruct.string.call(void 0, ),
  slot: _superstruct.number.call(void 0, ),
  err: TransactionErrorResult,
  memo: _superstruct.nullable.call(void 0, _superstruct.string.call(void 0, )),
...
var AccountNotificationResult = _superstruct.type.call(void 0, {
  subscription: _superstruct.number.call(void 0, ),
  result: notificationResultAndContext(AccountInfoResult)
});
var ProgramAccountInfoResult = _superstruct.type.call(void 0, {
  pubkey: PublicKeyFromString,
  account: AccountInfoResult
});
var ProgramAccountNotificationResult = _superstruct.type.call(void 0, {
  subscription: _superstruct.number.call(void 0, ),
  result: notificationResultAndContext(ProgramAccountInfoResult)
});
var SlotInfoResult = _superstruct.type.call(void 0, {
  parent: _superstruct.number.call(void 0, ),
  slot: _superstruct.number.call(void 0, ),
  root: _superstruct.number.call(void 0, )
});
var SlotNotificationResult = _superstruct.type.call(void 0, {
  subscription: _superstruct.number.call(void 0, ),
  result: SlotInfoResult
});
var SlotUpdateResult = _superstruct.union.call(void 0, [_superstruct.type.call(void 0, {
  type: _superstruct.union.call(void 0, [_superstruct.literal.call(void 0, "firstShredReceived"), _superstruct.literal.call(void 0, "completed"), _superstruct.literal.call(void 0, "optimisticConfirmation"), _superstruct.literal.call(void 0, "root")]),
  slot: _superstruct.number.call(void 0, ),
  timestamp: _superstruct.number.call(void 0, )
}), _superstruct.type.call(void 0, {
  type: _superstruct.literal.call(void 0, "createdBank"),
  parent: _superstruct.number.call(void 0, ),
  slot: _superstruct.number.call(void 0, ),
  timestamp: _superstruct.number.call(void 0, )
}), _superstruct.type.call(void 0, {
  type: _superstruct.literal.call(void 0, "frozen"),
  slot: _superstruct.number.call(void 0, ),
  timestamp: _superstruct.number.call(void 0, ),
  stats: _superstruct.type.call(void 0, {
    numTransactionEntries: _superstruct.number.call(void 0, ),
    numSuccessfulTransactions: _superstruct.number.call(void 0, ),
    numFailedTransactions: _superstruct.number.call(void 0, ),
    maxTransactionsPerEntry: _superstruct.number.call(void 0, )
  })
}), _superstruct.type.call(void 0, {
  type: _superstruct.literal.call(void 0, "dead"),
  slot: _superstruct.number.call(void 0, ),
  timestamp: _superstruct.number.call(void 0, ),
  err: _superstruct.string.call(void 0, )
})]);
var SlotUpdateNotificationResult = _superstruct.type.call(void 0, {
  subscription: _superstruct.number.call(void 0, ),
  result: SlotUpdateResult
});
var SignatureNotificationResult = _superstruct.type.call(void 0, {
  subscription: _superstruct.number.call(void 0, ),
  result: notificationResultAndContext(_superstruct.union.call(void 0, [SignatureStatusResult, SignatureReceivedResult]))
});
var RootNotificationResult = _superstruct.type.call(void 0, {
  subscription: _superstruct.number.call(void 0, ),
  result: _superstruct.number.call(void 0, )
});
var ContactInfoResult = _superstruct.type.call(void 0, {
  pubkey: _superstruct.string.call(void 0, ),
  gossip: _superstruct.nullable.call(void 0, _superstruct.string.call(void 0, )),
  tpu: _superstruct.nullable.call(void 0, _superstruct.string.call(void 0, )),
  rpc: _superstruct.nullable.call(void 0, _superstruct.string.call(void 0, )),
  version: _superstruct.nullable.call(void 0, _superstruct.string.call(void 0, ))
});
var VoteAccountInfoResult = _superstruct.type.call(void 0, {
  votePubkey: _superstruct.string.call(void 0, ),
  nodePubkey: _superstruct.string.call(void 0, ),
  activatedStake: _superstruct.number.call(void 0, ),
  epochVoteAccount: _superstruct.boolean.call(void 0, ),
  epochCredits: _superstruct.array.call(void 0, _superstruct.tuple.call(void 0, [_superstruct.number.call(void 0, ), _superstruct.number.call(void 0, ), _superstruct.number.call(void 0, )])),
  commission: _superstruct.number.call(void 0, ),
  lastVote: _superstruct.number.call(void 0, ),
  rootSlot: _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, ))
});
var GetVoteAccounts = jsonRpcResult(_superstruct.type.call(void 0, {
  current: _superstruct.array.call(void 0, VoteAccountInfoResult),
  delinquent: _superstruct.array.call(void 0, VoteAccountInfoResult)
}));
var ConfirmationStatus = _superstruct.union.call(void 0, [_superstruct.literal.call(void 0, "processed"), _superstruct.literal.call(void 0, "confirmed"), _superstruct.literal.call(void 0, "finalized")]);
var SignatureStatusResponse = _superstruct.type.call(void 0, {
  slot: _superstruct.number.call(void 0, ),
  confirmations: _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, )),
  err: TransactionErrorResult,
  confirmationStatus: _superstruct.optional.call(void 0, ConfirmationStatus)
});
var GetSignatureStatusesRpcResult = jsonRpcResultAndContext(_superstruct.array.call(void 0, _superstruct.nullable.call(void 0, SignatureStatusResponse)));
var GetMinimumBalanceForRentExemptionRpcResult = jsonRpcResult(_superstruct.number.call(void 0, ));
var AddressTableLookupStruct = _superstruct.type.call(void 0, {
  accountKey: PublicKeyFromString,
  writableIndexes: _superstruct.array.call(void 0, _superstruct.number.call(void 0, )),
  readonlyIndexes: _superstruct.array.call(void 0, _superstruct.number.call(void 0, ))
});
var ConfirmedTransactionResult = _superstruct.type.call(void 0, {
  signatures: _superstruct.array.call(void 0, _superstruct.string.call(void 0, )),
  message: _superstruct.type.call(void 0, {
    accountKeys: _superstruct.array.call(void 0, _superstruct.string.call(void 0, )),
    header: _superstruct.type.call(void 0, {
      numRequiredSignatures: _superstruct.number.call(void 0, ),
      numReadonlySignedAccounts: _superstruct.number.call(void 0, ),
      numReadonlyUnsignedAccounts: _superstruct.number.call(void 0, )
    }),
    instructions: _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
      accounts: _superstruct.array.call(void 0, _superstruct.number.call(void 0, )),
      data: _superstruct.string.call(void 0, ),
      programIdIndex: _superstruct.number.call(void 0, )
    })),
    recentBlockhash: _superstruct.string.call(void 0, ),
    addressTableLookups: _superstruct.optional.call(void 0, _superstruct.array.call(void 0, AddressTableLookupStruct))
  })
});
var AnnotatedAccountKey = _superstruct.type.call(void 0, {
  pubkey: PublicKeyFromString,
  signer: _superstruct.boolean.call(void 0, ),
  writable: _superstruct.boolean.call(void 0, ),
  source: _superstruct.optional.call(void 0, _superstruct.union.call(void 0, [_superstruct.literal.call(void 0, "transaction"), _superstruct.literal.call(void 0, "lookupTable")]))
});
var ConfirmedTransactionAccountsModeResult = _superstruct.type.call(void 0, {
  accountKeys: _superstruct.array.call(void 0, AnnotatedAccountKey),
  signatures: _superstruct.array.call(void 0, _superstruct.string.call(void 0, ))
});
var ParsedInstructionResult = _superstruct.type.call(void 0, {
  parsed: _superstruct.unknown.call(void 0, ),
  program: _superstruct.string.call(void 0, ),
  programId: PublicKeyFromString
});
var RawInstructionResult = _superstruct.type.call(void 0, {
  accounts: _superstruct.array.call(void 0, PublicKeyFromString),
  data: _superstruct.string.call(void 0, ),
  programId: PublicKeyFromString
});
var InstructionResult = _superstruct.union.call(void 0, [RawInstructionResult, ParsedInstructionResult]);
var UnknownInstructionResult = _superstruct.union.call(void 0, [_superstruct.type.call(void 0, {
  parsed: _superstruct.unknown.call(void 0, ),
  program: _superstruct.string.call(void 0, ),
  programId: _superstruct.string.call(void 0, )
}), _superstruct.type.call(void 0, {
...
var ParsedOrRawInstruction = _superstruct.coerce.call(void 0, InstructionResult, UnknownInstructionResult, (value) => {
  if ("accounts" in value) {
    return _superstruct.create.call(void 0, value, RawInstructionResult);
  } else {
    return _superstruct.create.call(void 0, value, ParsedInstructionResult);
  }
});
var ParsedConfirmedTransactionResult = _superstruct.type.call(void 0, {
  signatures: _superstruct.array.call(void 0, _superstruct.string.call(void 0, )),
  message: _superstruct.type.call(void 0, {
    accountKeys: _superstruct.array.call(void 0, AnnotatedAccountKey),
    instructions: _superstruct.array.call(void 0, ParsedOrRawInstruction),
    recentBlockhash: _superstruct.string.call(void 0, ),
    addressTableLookups: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, AddressTableLookupStruct)))
  })
});
var TokenBalanceResult = _superstruct.type.call(void 0, {
  accountIndex: _superstruct.number.call(void 0, ),
  mint: _superstruct.string.call(void 0, ),
  owner: _superstruct.optional.call(void 0, _superstruct.string.call(void 0, )),
  programId: _superstruct.optional.call(void 0, _superstruct.string.call(void 0, )),
  uiTokenAmount: TokenAmountResult
});
var LoadedAddressesResult = _superstruct.type.call(void 0, {
  writable: _superstruct.array.call(void 0, PublicKeyFromString),
  readonly: _superstruct.array.call(void 0, PublicKeyFromString)
});
var ConfirmedTransactionMetaResult = _superstruct.type.call(void 0, {
  err: TransactionErrorResult,
  fee: _superstruct.number.call(void 0, ),
  innerInstructions: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
    index: _superstruct.number.call(void 0, ),
    instructions: _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
      accounts: _superstruct.array.call(void 0, _superstruct.number.call(void 0, )),
  ...
  preBalances: _superstruct.array.call(void 0, _superstruct.number.call(void 0, )),
  postBalances: _superstruct.array.call(void 0, _superstruct.number.call(void 0, )),
  logMessages: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, _superstruct.string.call(void 0, )))),
  preTokenBalances: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, TokenBalanceResult))),
  postTokenBalances: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, TokenBalanceResult))),
  loadedAddresses: _superstruct.optional.call(void 0, LoadedAddressesResult),
  computeUnitsConsumed: _superstruct.optional.call(void 0, _superstruct.number.call(void 0, ))
});
var ParsedConfirmedTransactionMetaResult = _superstruct.type.call(void 0, {
  err: TransactionErrorResult,
  fee: _superstruct.number.call(void 0, ),
  innerInstructions: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
    index: _superstruct.number.call(void 0, ),
...
var TransactionVersionStruct = _superstruct.union.call(void 0, [_superstruct.literal.call(void 0, 0), _superstruct.literal.call(void 0, "legacy")]);
var RewardsResult = _superstruct.type.call(void 0, {
  pubkey: _superstruct.string.call(void 0, ),
  lamports: _superstruct.number.call(void 0, ),
  postBalance: _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, )),
  rewardType: _superstruct.nullable.call(void 0, _superstruct.string.call(void 0, )),
  commission: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, )))
});
var GetBlockRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  previousBlockhash: _superstruct.string.call(void 0, ),
  parentSlot: _superstruct.number.call(void 0, ),
  transactions: _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
    transaction: ConfirmedTransactionResult,
    meta: _superstruct.nullable.call(void 0, ConfirmedTransactionMetaResult),
    version: _superstruct.optional.call(void 0, TransactionVersionStruct)
  })),
  rewards: _superstruct.optional.call(void 0, _superstruct.array.call(void 0, RewardsResult)),
  blockTime: _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, )),
  blockHeight: _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, ))
})));
var GetNoneModeBlockRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  previousBlockhash: _superstruct.string.call(void 0, ),
  parentSlot: _superstruct.number.call(void 0, ),
  rewards: _superstruct.optional.call(void 0, _superstruct.array.call(void 0, RewardsResult)),
...
var GetAccountsModeBlockRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  previousBlockhash: _superstruct.string.call(void 0, ),
  parentSlot: _superstruct.number.call(void 0, ),
  transactions: _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
...
var GetParsedBlockRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  previousBlockhash: _superstruct.string.call(void 0, ),
  parentSlot: _superstruct.number.call(void 0, ),
  transactions: _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
    transaction: ParsedConfirmedTransactionResult,
    meta: _superstruct.nullable.call(void 0, ParsedConfirmedTransactionMetaResult),
    version: _superstruct.optional.call(void 0, TransactionVersionStruct)
  })),
  rewards: _superstruct.optional.call(void 0, _superstruct.array.call(void 0, RewardsResult)),
  blockTime: _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, )),
...
var GetParsedAccountsModeBlockRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  previousBlockhash: _superstruct.string.call(void 0, ),
  parentSlot: _superstruct.number.call(void 0, ),
  transactions: _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
...
var GetParsedNoneModeBlockRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  previousBlockhash: _superstruct.string.call(void 0, ),
  parentSlot: _superstruct.number.call(void 0, ),
  rewards: _superstruct.optional.call(void 0, _superstruct.array.call(void 0, RewardsResult)),
...
var GetConfirmedBlockRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  previousBlockhash: _superstruct.string.call(void 0, ),
  parentSlot: _superstruct.number.call(void 0, ),
  transactions: _superstruct.array.call(void 0, _superstruct.type.call(void 0, {
...
var GetBlockSignaturesRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  previousBlockhash: _superstruct.string.call(void 0, ),
  parentSlot: _superstruct.number.call(void 0, ),
  signatures: _superstruct.array.call(void 0, _superstruct.string.call(void 0, )),
...
var GetTransactionRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  slot: _superstruct.number.call(void 0, ),
  meta: _superstruct.nullable.call(void 0, ConfirmedTransactionMetaResult),
  blockTime: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, ))),
  transaction: ConfirmedTransactionResult,
...
var GetParsedTransactionRpcResult = jsonRpcResult(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  slot: _superstruct.number.call(void 0, ),
  transaction: ParsedConfirmedTransactionResult,
  meta: _superstruct.nullable.call(void 0, ParsedConfirmedTransactionMetaResult),
  blockTime: _superstruct.optional.call(void 0, _superstruct.nullable.call(void 0, _superstruct.number.call(void 0, ))),
...
var GetLatestBlockhashRpcResult = jsonRpcResultAndContext(_superstruct.type.call(void 0, {
  blockhash: _superstruct.string.call(void 0, ),
  lastValidBlockHeight: _superstruct.number.call(void 0, )
}));
var IsBlockhashValidRpcResult = jsonRpcResultAndContext(_superstruct.boolean.call(void 0, ));
var PerfSampleResult = _superstruct.type.call(void 0, {
  slot: _superstruct.number.call(void 0, ),
  numTransactions: _superstruct.number.call(void 0, ),
  numSlots: _superstruct.number.call(void 0, ),
  samplePeriodSecs: _superstruct.number.call(void 0, )
});
var GetRecentPerformanceSamplesRpcResult = jsonRpcResult(_superstruct.array.call(void 0, PerfSampleResult));
var GetFeeCalculatorRpcResult = jsonRpcResultAndContext(_superstruct.nullable.call(void 0, _superstruct.type.call(void 0, {
  feeCalculator: _superstruct.type.call(void 0, {
    lamportsPerSignature: _superstruct.number.call(void 0, )
  })
})));
var RequestAirdropRpcResult = jsonRpcResult(_superstruct.string.call(void 0, ));
var SendTransactionRpcResult = jsonRpcResult(_superstruct.string.call(void 0, ));
var LogsResult = _superstruct.type.call(void 0, {
  err: TransactionErrorResult,
  logs: _superstruct.array.call(void 0, _superstruct.string.call(void 0, )),
  signature: _superstruct.string.call(void 0, )
});
var LogsNotificationResult = _superstruct.type.call(void 0, {
...
var Keypair = class {
  ...
  constructor(keypair) {
    ...
    this._keypair = _nullishCoalesce(keypair, () => ( generateKeypair()));
  }
  /**
...
var AddressLookupTableProgram = class {
  ...
  static createLookupTable(params) {
    const [lookupTableAddress, bumpSeed] = PublicKey.findProgramAddressSync([params.authority.toBuffer(), _bigintbuffer.toBufferLE.call(void 0, BigInt(params.recentSlot), 8)], this.programId);
...
var ecdsaSign = (msgHash, privKey) => {
  const signature = _secp256k1.secp256k1.sign(msgHash, privKey);
  return [signature.toCompactRawBytes(), signature.recovery];
};
_secp256k1.secp256k1.utils.isValidPrivateKey;
var publicKeyCreate = _secp256k1.secp256k1.getPublicKey;
...
var Secp256k1Program = class {
  ...
  static publicKeyToEthAddress(publicKey2) {
    ...
    try {
      return _buffer.Buffer.from(_sha3.keccak_256.call(void 0, toBuffer(publicKey2))).slice(-ETHEREUM_ADDRESS_BYTES);
    } catch (error) {
  ...
  static createInstructionWithPrivateKey(params) {
    ...
    try {
      ...
      const messageHash = _buffer.Buffer.from(_sha3.keccak_256.call(void 0, toBuffer(message)));
...
var InfoString = _superstruct.type.call(void 0, {
  name: _superstruct.string.call(void 0, ),
  website: _superstruct.optional.call(void 0, _superstruct.string.call(void 0, )),
  details: _superstruct.optional.call(void 0, _superstruct.string.call(void 0, )),
  iconUrl: _superstruct.optional.call(void 0, _superstruct.string.call(void 0, )),
  keybaseUsername: _superstruct.optional.call(void 0, _superstruct.string.call(void 0, ))
});
...
var _borsh3 = require('@coral-xyz/borsh');
...
var ClockLayout = _borsh3.struct.call(void 0, [
  _borsh3.u64.call(void 0, "slot"),
  _borsh3.i64.call(void 0, "epochStartTimestamp"),
  _borsh3.u64.call(void 0, "epoch"),
  _borsh3.u64.call(void 0, "leaderScheduleEpoch"),
  _borsh3.i64.call(void 0, "unixTimestamp")
]);
...
function createProgram(connection, opt) {
  ...
  return new (0, _anchor.Program)(
    { ...alpha_vault_default, address: PROGRAM_ID[_optionalChain([opt, 'optionalAccess', _5 => _5.cluster]) || "mainnet-beta"] },
    provider
  );
}
function createDlmmProgram(connection, opt) {
  ...
  return new (0, _anchor0280.Program)(
    _dlmm.IDL,
    _dlmm.LBCLMM_PROGRAM_IDS[_optionalChain([opt, 'optionalAccess', _6 => _6.cluster]) || "mainnet-beta"],
    provider
  );
}
...
var fillDlmmTransaction = async (program, vaultKey, vault, payer, opt) => {
  ...
  const connection = program.provider.connection;
  const cluster = _nullishCoalesce(_optionalChain([opt, 'optionalAccess', _7 => _7.cluster]), () => ( "mainnet-beta"));
  ...
  const dlmmProgramId = new PublicKey(_dlmm.LBCLMM_PROGRAM_IDS[cluster]);
...
var fillDammTransaction = async (program, vaultKey, vault, payer, opt) => {
  ...
  const pool = await _dynamicammsdk2.default.create(connection, vault.pool, {
    cluster: _nullishCoalesce(_optionalChain([opt, 'optionalAccess', _8 => _8.cluster]), () => ( "mainnet-beta"))
  });
  const [crankFeeWhitelist] = deriveCrankFeeWhitelist(payer, program.programId);
  const crankFeeWhitelistAccount = await connection.getAccountInfo(crankFeeWhitelist);
  const preInstructions = [];
...
function shouldStopFilling(vault) {
  switch (vault.vaultMode) {
    case 0 /* PRORATA */:
      const maxSwappableAmount = _bnjs2.default.min(vault.totalDeposit, vault.maxBuyingCap);
...
function bufDedup(elements) {
  return elements.filter((el, idx) => {
    return idx === 0 || !_optionalChain([elements, 'access', _9 => _9[idx - 1], 'optionalAccess', _10 => _10.equals, 'call', _11 => _11(el)]);
  });
}
...
var MerkleTree = class {
  ...
  getLayers(elements) {
    ...
    while ((_nullishCoalesce(_optionalChain([layers, 'access', _12 => _12[layers.length - 1], 'optionalAccess', _13 => _13.length]), () => ( 0))) > 1) {
  ...
  getRoot() {
    const root = _optionalChain([this, 'access', _14 => _14._layers, 'access', _15 => _15[this._layers.length - 1], 'optionalAccess', _16 => _16[0]]);
...
var AlphaVault = class {
  ...
  static async create(connection, vaultAddress, opt) {
    ...
    const cluster = _optionalChain([opt, 'optionalAccess', _17 => _17.cluster]) || "mainnet-beta";
  ...
  getAvailableDepositQuota(escrow, merkleProof) {
    const deposited = _nullishCoalesce(_optionalChain([escrow, 'optionalAccess', _18 => _18.totalDeposit]), () => ( new (0, _anchor.BN)(0)));
